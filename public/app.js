...
function renderBookings(list){
  ...
  return`<div class="bcard ${isIn?'':'out'}" style="animation-delay:${Math.min(i,20)*0.02}s"><div class="bcard-row" onclick="openBookingDetail(${idx})"><div class="bcard-time">${checked?'<div class="bt-checked">✔</div>':''}<div class="bt-icon">🚐</div><div class="bt-time">${esc(time)||'—'}</div>${flight?`<div class="bt-flight">${esc(flight)}</div>`:''}</div><div class="bcard-body"><div class="bcard-top"><span class="bcard-badge pk">${cn}</span>${providerTag}</div><div><span class="bcard-plate">${esc(b.plate)||'—'}</span><span class="bcard-code">#${esc(b.external_id||'')}${b.uid&&b.uid!==b.external_id?' '+esc(b.uid):''}</span></div><div class="bcard-name">${esc(b.name)||'—'}</div>${b.car?`<div class="bcard-meta">🚗 ${esc(b.car)}</div>`:''}</div><div class="bcard-right">${b.pax?`<div class="bcard-pax">${b.pax} 👥</div>`:''}${printed?'<div class="bcard-printed">🖨</div>':''}${b.price?`<div class="bcard-price">${b.price.toFixed(2)}€</div>`:'<div class="bcard-price">0,00€</div>'}</div></div></div>`;}).join('');
}
