...
async function initApp(){
  ...
  autoSync();
  updateSyncBadge();
  // Prefetch für Offline (im Hintergrund)
  prefetchBookingsForOffline();
}
...
function updateHeaderGreeting() {
  ...
  else if (hour < 18) greeting = 'Hallo';
  else greeting = 'Guten Abend';
  el.textContent = `${greeting}, ${firstName}!`;
}

// ─── Offline Prefetch ───────────────────────────────────────────────────────
async function prefetchBookingsForOffline() {
  if (isOffline) return;
  
  const today = new Date();
  const dates = [];
  
  // 7 Tage zurück + heute + 14 Tage voraus = 22 Tage
  for (let i = -7; i <= 14; i++) {
  ...
  // Im Hintergrund für beide Firmen laden
  const companies = ['parkking', 'psfmsf'];
  
  for (const company of companies) {
    for (const date of dates) {
      // Prüfe ob schon gecacht
      const cached = await OfflineStore.getCachedBookings(company, date);
      const cacheAge = cached ? (Date.now() - cached.cachedAt) : Infinity;
      
      // Nur laden wenn nicht gecacht oder älter als 1 Stunde
      if (cacheAge > 60 * 60 * 1000) {
        try {
          const r = await fetch(`/api/bookings?company=${company}&date=${date}`, { headers: ah() });
          if (r.ok) {
            const data = await r.json();
            await OfflineStore.cacheBookings(company, date, data);
          }
        } catch (e) {
  ...
  console.log('[Offline] Prefetch abgeschlossen: 22 Tage für beide Firmen');
}
...
function handleSearch() {
  ...
  if (q.length < 2) {
    // Kurze Suche = normale lokale Filterung
    globalSearchResults = [];
    filterBookings();
    return;
  }
  
  // Warte 300ms dann globale Suche
  searchTimeout = setTimeout(() => searchGlobal(q), 300);
}

async function searchGlobal(q) {
  try {
    const r = await fetch('/api/bookings/search?q=' + encodeURIComponent(q), { headers: ah() });
    const data = await r.json();
    globalSearchResults = data.bookings || [];
    
    if (globalSearchResults.length > 0) {
      // Zeige globale Suchergebnisse
      renderBookings(globalSearchResults);
      document.getElementById('statusBar').innerHTML = `<div class="status-chip" style="background:var(--blue-bg);color:var(--blue)">🔍 ${globalSearchResults.length} Treffer (alle Tage)</div>`;
    } else {
      // Fallback zu lokaler Suche
      filterBookings();
    }
  } catch (e) {
    // Bei Fehler lokale Suche
    filterBookings();
  }
}
...
function filterBookings(){
  ...
  // Wenn globale Suche aktiv, nicht filtern
  if (globalSearchResults.length > 0 && q.length >= 2) {
    return;
  }
  ...
  // Textsuche (case-insensitive, auch Leerzeichen ignorieren)
  if (q) {
    const qNorm = q.replace(/[\s-]/g, ''); // Entferne Leerzeichen und Bindestriche
    filtered = filtered.filter(b => {
      const plateNorm = (b.plate || '').toLowerCase().replace(/[\s-]/g, '');
      const nameNorm = (b.name || '').toLowerCase();
      return plateNorm.includes(qNorm) || nameNorm.includes(q);
    });
  }
  ...
  if (hideChecked) {
    filtered = filtered.filter(b => {
      if (b.type === 'in') {
        return !b.checked_in_at;
      } else {
        return !b.checked_out_at;
      }
    });
  }
...
async function printAllLabels() {
  // Alle Annahmen (grün) für den ausgewählten Tag
  const arrivals = allBookings.filter(b => b.type === 'in');
  
  if (arrivals.length === 0) {
    showToast('Keine Annahmen für heute ⚠️');
    return;
  }
  
  if (!confirm(`${arrivals.length} Etiketten für alle Annahmen erstellen?\n\nDies kann einen Moment dauern.`)) {
    return;
  }
  
  showToast(`🏷️ Erstelle ${arrivals.length} Etiketten...`);
  
  // Erstelle alle Labels auf einmal
  const labels = [];
  for (const b of arrivals) {
    const labelData = {
      ...
      pax: b.pax || 1,
      ref: b.ref || '',
      parkplatz: b.parkplatz || ''
    };
    labels.push(labelData);
  }
  
  // Öffne Print-Seite mit allen Labels
  openMultiLabelPrint(labels);
}

function openMultiLabelPrint(labels) {
  const LABEL_W = 57, LABEL_H = 32, SCALE = 4;
  const W = LABEL_W * SCALE, H = LABEL_H * SCALE;
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    allCanvasHTML += `<div style="page-break-after: always; margin-bottom: 10px;"><canvas id="${canvasId}" width="${W}" height="${H}"></canvas></div>`;
  }
  ...
  printWindow.document.write(`
    ...
    <head>
      ...
      <style>
        ...
        @media print { 
          body { padding: 0; }
          canvas { border: none; margin: 0; }
          div { page-break-after: always; margin: 0 !important; }
        }
        .header { text-align: center; margin-bottom: 20px; }
        .print-btn { padding: 15px 30px; font-size: 18px; background: #22c55e; color: white; border: none; border-radius: 8px; cursor: pointer; margin: 10px; }
      </style>
    </head>
    <body>
      <div class="header">
        <h2>🏷️ ${labels.length} Etiketten</h2>
        <button class="print-btn" onclick="window.print()">🖨️ Alle drucken</button>
        <button class="print-btn" style="background:#666" onclick="window.close()">Schließen</button>
      </div>
      ${allCanvasHTML}
      <script>
        const labels = ${JSON.stringify(labels)};
        
        function drawLabel(canvas, b) {
          ...
          // Kennzeichen
          ctx.fillStyle = '#000';
          ctx.font = 'bold ' + (H * 0.28) + 'px Arial';
          ctx.textAlign = 'center';
          ctx.fillText(b.plate, W/2, H * 0.28);
          
          // Name
          ctx.font = (H * 0.14) + 'px Arial';
          ctx.fillText(b.name.substring(0, 25), W/2, H * 0.46);
          
          // Datum
          ctx.font = (H * 0.13) + 'px Arial';
          const dateStr = b.date + ' ' + b.time + ' → ' + b.depDate + ' ' + b.depTime;
          ctx.fillText(dateStr, W/2, H * 0.64);
          
          // Unten: Pax + Ref + Parkplatz
          ctx.font = (H * 0.11) + 'px Arial';
          const bottomStr = b.pax + ' Pax | ' + b.ref + (b.parkplatz ? ' | ' + b.parkplatz : '');
          ctx.fillText(bottomStr, W/2, H * 0.82);
        }
        
        labels.forEach((b, i) => {
          const canvas = document.getElementById('labelCanvas' + i);
          drawLabel(canvas, b);
        });
      <\/script>
    </body>
...
async function endUnplannedShift() {
  try {
    ...
    const loc = await checkLocation();
    if (!loc.ok) {
      alert(`❌ Du bist nicht am Arbeitsort!\n\nDu bist ${Math.round(loc.distance)}m entfernt.\nCheck-out nur möglich bei: ${loc.location?.name || 'Arbeitsort'}`);
      return;
    }
...
async function checkLocation() {
  // Lade Arbeitsorte von API (falls noch nicht geladen)
  let locations = workLocations;
  if (!locations || locations.length === 0) {
    try {
      const r = await fetch('/api/work-locations', { headers: ah() });
      locations = await r.json();
      workLocations = locations;
    } catch (e) {
      // Fallback zu default
      locations = [{ name: 'Klövesteen 21', latitude: 53.6847, longitude: 9.8908, radius_m: 3000, active: 1 }];
    }
  }
  ...
  if (activeLocations.length === 0) {
    // Keine Arbeitsorte definiert = Check deaktiviert
    return { ok: true, distance: 0, location: null };
  }
      ...
      (pos) => {
        ...
        for (const loc of activeLocations) {
          ...
          if (dist <= loc.radius_m) {
            resolve({ ok: true, distance: dist, location: loc });
            return;
          }
        }
        // Kein Arbeitsort in Reichweite - zeige nächsten
        let nearest = activeLocations[0];
        let nearestDist = getDistanceFromLatLonInMeters(
          pos.coords.latitude, pos.coords.longitude,
          nearest.latitude, nearest.longitude
        );
        for (const loc of activeLocations.slice(1)) {
          const dist = getDistanceFromLatLonInMeters(
            pos.coords.latitude, pos.coords.longitude,
            loc.latitude, loc.longitude
          );
        ...
        resolve({ ok: false, distance: nearestDist, location: nearest });
      },
      (err) => reject(err),
...
async function shiftCheckin(shiftId, isLate) {
  try {
    ...
    const loc = await checkLocation();
    if (!loc.ok) {
      alert(`❌ Du bist nicht am Arbeitsort!\n\nDu bist ${Math.round(loc.distance)}m entfernt.\nCheck-in nur möglich bei: ${loc.location?.name || 'Arbeitsort'} (${loc.location?.address || ''})`);
      return;
    }
...
async function loadAdminData(){await Promise.all([loadWorkLocations(),loadUsers(),loadAdminTasks(),loadAdminTemplates(),loadMonthlyHours()]);}
...
function renderWorkLocations() {
  const el = document.getElementById('workLocationsList');
  if (!el) return;
  if (!workLocations.length) {
    el.innerHTML = '<div class="task-empty">Keine Arbeitsorte definiert.</div>';
    return;
  }
  el.innerHTML = workLocations.map(loc => `
    <div class="admin-card" ${!loc.active ? 'style="opacity:0.5"' : ''}>
      <div class="admin-card-row">
        <div class="admin-avatar" style="background:var(--green-bg);color:var(--green)">📍</div>
        <div class="admin-card-info">
          <div class="admin-card-name">${esc(loc.name)} ${!loc.active ? '<span class="role-badge role-inactive">Inaktiv</span>' : ''}</div>
          <div class="admin-card-sub">${esc(loc.address || '')} · ${loc.radius_m}m Radius</div>
          <div class="admin-card-sub" style="font-family:'JetBrains Mono',monospace;font-size:10px">${loc.latitude}, ${loc.longitude}</div>
        </div>
      </div>
      <div class="admin-actions">
        <button class="btn btn-white btn-xs" onclick="openEditWorkLocation(${loc.id})">Bearbeiten</button>
        <button class="btn btn-xs" style="background:var(--red-bg);color:var(--red);border:none" onclick="deleteWorkLocation(${loc.id},'${esc(loc.name)}')">Löschen</button>
      </div>
    </div>
  `).join('');
}

function openEditWorkLocation(id) {
  const loc = workLocations.find(l => l.id === id);
  if (!loc) return;
  openModal('editWorkLocation', loc);
}

async function deleteWorkLocation(id, name) {
  if (!confirm(`Arbeitsort "${name}" wirklich löschen?`)) return;
  try {
    await fetch('/api/work-locations/' + id, { method: 'DELETE', headers: ah() });
...
async function createWorkLocation() {
  ...
  const latitude = parseFloat(document.getElementById('wlLat').value);
  const longitude = parseFloat(document.getElementById('wlLng').value);
  const radius_m = parseInt(document.getElementById('wlRadius').value) || 500;
  
  if (!name || !latitude || !longitude) {
    showToast('Name und Koordinaten erforderlich ⚠️');
    return;
  }
  ...
  try {
    await fetch('/api/work-locations', {
      method: 'POST',
      headers: ah(),
      body: JSON.stringify({ name, address, latitude, longitude, radius_m })
    });
    showToast('Arbeitsort erstellt ✓');
    closeModal();
    loadWorkLocations();
  } catch (e) { showToast('Fehler ⚠️'); }
}

async function updateWorkLocation(id) {
  const name = document.getElementById('wlName').value.trim();
  const address = document.getElementById('wlAddress').value.trim();
  const latitude = parseFloat(document.getElementById('wlLat').value);
  const longitude = parseFloat(document.getElementById('wlLng').value);
  ...
  try {
    ...
    showToast('Arbeitsort aktualisiert ✓');
    closeModal();
    loadWorkLocations();
  } catch (e) { showToast('Fehler ⚠️'); }
}
...
async function useCurrentLocation() {
  if (!navigator.geolocation) {
    showToast('GPS nicht verfügbar ⚠️');
    return;
  }
  showToast('📍 Standort wird ermittelt...');
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      document.getElementById('wlLat').value = pos.coords.latitude.toFixed(6);
      document.getElementById('wlLng').value = pos.coords.longitude.toFixed(6);
      showToast('Koordinaten übernommen ✓');
    },
    (err) => showToast('GPS-Fehler: ' + err.message + ' ⚠️'),
    { enableHighAccuracy: true, timeout: 10000 }
  );
}
...
function openModal(type,data){
  ...
  else if(type==='addWorkLocation'){content.innerHTML=`<div class="modal-title">Neuer Arbeitsort <button class="modal-close" onclick="closeModal()">✕</button></div>
    <div class="form-group"><label class="form-label">Name *</label><input class="form-input" id="wlName" placeholder="z.B. Hauptstandort"></div>
    <div class="form-group"><label class="form-label">Adresse</label><input class="form-input" id="wlAddress" placeholder="Straße, PLZ Ort"></div>
    <div style="display:flex;gap:8px">
      <div class="form-group" style="flex:1"><label class="form-label">Latitude *</label><input class="form-input" id="wlLat" type="number" step="0.000001" placeholder="53.6847"></div>
      <div class="form-group" style="flex:1"><label class="form-label">Longitude *</label><input class="form-input" id="wlLng" type="number" step="0.000001" placeholder="9.8908"></div>
    </div>
    <button class="btn btn-outline btn-sm btn-full" style="margin-bottom:12px" onclick="useCurrentLocation()">📍 Aktuellen Standort verwenden</button>
    <div class="form-group"><label class="form-label">Radius (Meter)</label><input class="form-input" id="wlRadius" type="number" value="500" min="50" max="10000"></div>
    <button class="btn btn-accent btn-full" onclick="createWorkLocation()">Arbeitsort erstellen</button>`;}
  else if(type==='editWorkLocation'){content.innerHTML=`<div class="modal-title">Arbeitsort bearbeiten <button class="modal-close" onclick="closeModal()">✕</button></div>
    <div class="form-group"><label class="form-label">Name *</label><input class="form-input" id="wlName" value="${esc(data.name)}"></div>
    <div class="form-group"><label class="form-label">Adresse</label><input class="form-input" id="wlAddress" value="${esc(data.address||'')}"></div>
    <div style="display:flex;gap:8px">
      <div class="form-group" style="flex:1"><label class="form-label">Latitude *</label><input class="form-input" id="wlLat" type="number" step="0.000001" value="${data.latitude}"></div>
      <div class="form-group" style="flex:1"><label class="form-label">Longitude *</label><input class="form-input" id="wlLng" type="number" step="0.000001" value="${data.longitude}"></div>
    </div>
    <button class="btn btn-outline btn-sm btn-full" style="margin-bottom:12px" onclick="useCurrentLocation()">📍 Aktuellen Standort verwenden</button>
    <div class="form-group"><label class="form-label">Radius (Meter)</label><input class="form-input" id="wlRadius" type="number" value="${data.radius_m}" min="50" max="10000"></div>
    <button class="btn btn-accent btn-full" onclick="updateWorkLocation(${data.id})">Speichern</button>`;}}
